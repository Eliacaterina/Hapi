exports.register = function(server, options, next) {
  ...
  //here is where all the routes will go
  server.route([
  // HELLO WORLD
    {
      method:'GET',
    ...
    //GET ONE QUOTE
    {
      ...
      handler: function(request, reply){
        ...
        if (isNaN(id)) {
          return reply("Comeon, gimme a number bro").code(404);
        }
        reply( db[id] );
      }
    },
    ...
    },
    {
      ...
      path:'/quotes/{id}',
      handler: function(request,reply){
        var id=Number(request.params.id); //ENGLISH!!!! WHAT DO I WANT TO DO! I WANT YOU TO DELETE THIS SPECIFIC ELEMENT FROM THE ARRAY. 
    ...
    },
    {
      ...
      path:'/quotes/random',
      handler: function(request,reply){
       var randomNum = Math.floor(Math.random()*db.length);
       var randomQuote = db[randomNum];
       reply(randomQuote);
     }
    },
  ])
